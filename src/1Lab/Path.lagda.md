...
::: popup
The elimination principle for [[paths]], termed **path induction** or
simply **J**, says that, fixing an endpoint $x : A$, the identity family
$y \mapsto x \is y$ is inductively generated by the element
`refl`{.Agda}.

```agda
J : ∀ {ℓ₁ ℓ₂} {A : Type ℓ₁} {x : A} (P : ∀ y → x ≡ y → Type ℓ₂)
  → P x refl → ∀ {y} p → P y p
```
:::
...
::: popup
A **homotopy** between functions $f, g : (x : A) \to B(x)$ is a function
$$h : (x : A) \to f(x) \is g(x)$$. The principle of **function
extensionality**, `funext`{.Agda}, says that the type of homotopies $f
\sim g$ is [[equivalent|equivalence]] to the type of [[paths]] $f \is
g$.
:::
