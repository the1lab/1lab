...
```
  ...
  private abstract
    lemma
      ...
      ≡ y .F₁ (elem-hom f p) el
    lemma y {o = o} {o′ = o′} {el = el} {f = f} =
      J (λ s′ p → subst (λ e → ∣ y .F₀ (elem o′ e) ∣) p (y .F₁ (elem-hom f refl) el)
                ≡ y .F₁ (elem-hom f p) el)
        (transport-refl _)
```
-->
...
```agda
  slice→total : Functor (Slice Cat[ C ^op , Sets κ ] P) Cat[ (∫ C P) ^op , Sets _ ]
  slice→total = func where
    func : Functor _ Cat[ (∫ C P) ^op , Sets _ ]
    func .F₀ = slice-ob→presheaf
    func .F₁ {x} {y} h .η i (z , p) =
      h .map .η (i .ob) z , ap (λ e → e .η _ z) (h .commutes) ∙ p
...
```agda
    isom .rinv x =
      Functor-path (λ x → n-ua (Fibre-equiv _ _))
