...
$F_1(f)\id \To \id G_1(f)$.  Normally, this would be a case of removing
the identity on one side and introducing it on the other; in a
...
Because our naturator involves five occurrences of the associator, the
...
We omit the proof showing compatibility with the unitors, which is
similar in spirit.
...
since if the naturators of $\alpha$ and $\beta$ were invertible, then
the composite constructed above is invertible, too.
