...
is displayed over a map that is not definitionally equal to $f$, but
only identical to it by a law of the base category.  Therefore, the laws
of a displayed category $\cE \liesover \cB$ must be [[*dependent*|path
over]] on the corresponding laws in $\cB$. We introduce the notation
`_≡[_]_`{.Agda}, which will be written $f' \is_{p} g'$ when necessary,
to compare maps $f' : x' \to_{f} y'$ and $g' : x' \to_{g} y$ over an
identification $p : f \is g$.
