...
The types of STLC are generated by function types `` a `⇒ b ``, product
types `` a `× b ``, and a unit type `` `⊤ ``{.Agda}; Additionally, we
have an inclusion of objects from the target category to serve as "base
types", e.g. the natural numbers. The contexts are simply lists of
types.
...
Note that, for [practical applications], we need renaming neutrals to
...
[practical applications]: Cat.CartesianClosed.Solver.html
...
exprᵖ {h = h} (`$ f a) c = tyᵖ⟨ pushr (ap₂ ⟨_,_⟩ (idr _) refl ∙ sym (⟨⟩∘ h)) ⟩
...
```agda
  ...
  fixup = sym $
    ...
    ev ∘ ⟦ `λ f ⟧ᵉ ⊗₁ id ∘ ⟨ h ∘ ⟦ σ ⟧ʳ , m ⟩ ≡⟨ pulll (commutes _) ⟩
    ⟦ f ⟧ᵉ ∘ ⟨ h ∘ ⟦ σ ⟧ʳ , m ⟩               ∎
```
-->
...
The proof of soundness of normalisation gives an equation between what
we wrote above and a simple composition of projections. Since we're
working in a CCC, these terms are identical, and the proof of soundness
doesn't (in *any* sense) give a "reduction trace" from one to the
other--- only an opaque witness that they are the same.
