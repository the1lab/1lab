...
images f = im where
  ...
  the-img .dom = tt
  the-img .cod .fst = cut (Ker.kernel (Coker.coeq f))
  the-img .cod .snd {c} = kernels-are-subobjects C ∅ _ (Ker.has-is-kernel _)
```
...
```agda
  ...
  the-img .map ./-Hom.com = pulll (Ker.factors _) ∙ Coker.factors _
```
