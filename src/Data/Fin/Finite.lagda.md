...
Finite-Bool = fin {cardinality = 2} (inc enum) where
  enum : Bool ≃ Fin 2
  enum = Iso→Equiv λ where
    .fst false → 0
    .fst true → 1
    .snd .is-iso.inv fzero → false
    .snd .is-iso.inv (fsuc fzero) → true
    .snd .is-iso.rinv fzero → refl
    .snd .is-iso.rinv (fsuc fzero) → refl
    .snd .is-iso.linv true → refl
    .snd .is-iso.linv false → refl
